...
type MerkleTree struct {
	...
	merkleRoot []byte
	Leafs      []*Node
	hashMerge  func(left []byte, right []byte) []byte
}
...
func (n *Node) verifyNode() ([]byte, error) {
	...
	return n.Tree.hashMerge(leftBytes, rightBytes), nil
}
...
func (n *Node) calculateNodeHash() ([]byte, error) {
	...
	return n.Tree.hashMerge(n.Left.Hash, n.Right.Hash), nil
}
...
func buildIntermediate(nl []*Node, t *MerkleTree) (*Node, error) {
	...
	for i := 0; i < len(nl); i += 2 {
		...
		chash := t.hashMerge(nl[left].Hash, nl[right].Hash)
		n := &Node{
			Left:  nl[left],
			Right: nl[right],
			Hash:  chash,
	...
	for _, l := range m.Leafs {
		...
		if ok {
			...
			for currentParent != nil {
				...
				if bytes.Compare(m.hashMerge(leftBytes, rightBytes), currentParent.Hash) != 0 {
					return false, nil
				}
				currentParent = currentParent.Parent
			}
